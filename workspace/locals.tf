locals {
  all_users = contains(keys(databricks_user.users), data.databricks_current_user.me.user_name) ? merge(databricks_user.users, { "${data.databricks_current_user.me.user_name}" = data.databricks_current_user.me } ) : databricks_user.users
